...
- **Pipelines: `67,000+` supported, `75,000+` runs per week** (both documented). Both raised on 2026-09-18, from `64,000+` and `70,000+` — the inventory passed 67k and weekly runs passed 75k. Each rounds down to the last whole thousand it is safely past, so both stay true as the counts keep climbing. `100,000+` is still the **post-backfill forecast** — same category as the 8,000, so it is **not claimed**. Keep pipelines (inventory) and runs (throughput) distinct; the pair is what shows the system is both large and live. **Raise them together.** The two are reported separately but read as one sentence, and runs must stay clearly above pipelines: a week in which the average pipeline runs less than once is not the picture this claim is making, so a pipelines-only raise that narrows the gap is the thing to catch. (The 2026-09-18 pipelines raise briefly closed it to 67k against 70k before the runs figure caught up.)
- **The verb carries as much risk as the number** (added 2026-08-26). Three different relationships exist and must not be blurred:
  - **Supported** — we keep it running where it lives. True of the 67,000+ pipelines. **Not** "managed": consolidating them onto platform tooling via a unified pipeline model is *planned*, so "managed"/"instrumented" would be a forecast wearing a present-tense verb.
  - **Tracked** — the platform has visibility. True of the `13,000+` repositories today. CI/CD instrumentation across brownfield apps is still rolling out, so "under platform management" would overstate it.
...
- **Repos migrated stays `10,000+`, not `11,000+` or the tracked count.** Three separate populations, easy to conflate: ~11k **in** GHEC, of which `10,000+` were **migrated** (the rest net-new in Cloud); `13,000+` currently **tracked** by the platform; and a much smaller set actually **instrumented** with platform CI/CD tooling — which is not claimed at all until the rollout lands.
